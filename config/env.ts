export const ENV = {
  ...
  // Supabase URL (falls back to empty string if not in .env)
  SUPABASE_URL: process.env.EXPO_PUBLIC_SUPABASE_URL || '',
  
  // Supabase Anon Key (falls back to empty string if not in .env)
  SUPABASE_ANON_KEY: process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || '',

  // Spring Boot API Base URL (falls back to empty string if not in .env)
...
export const getSupabaseProjectRef = (): string => {
  const match = ENV.SUPABASE_URL ? ENV.SUPABASE_URL.match(/https:\/\/([a-z0-9]+)\.supabase/) : null;
